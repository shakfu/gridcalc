...
- **Headless mode: the solve/goal/sweep pipeline without a screen.** `gridcalc book.json` still opens the editor, but any operation flag turns the same command line into a batch run that never touches curses -- so the thing gridcalc is actually for, optimization with spreadsheet semantics, can be driven by cron, CI, a Makefile, or another program. Excel's Solver needs a human in a dialog and PuLP needs the model rewritten as code; this needs neither, because the model already lives in the sheet and the sheet is the input file.

  `--solve [MODEL|SPEC]` runs a saved model (`default` when bare) or an inline one, `--sens` and `--diagnose` add duals and failure explanations, `--sweep` re-solves across a right-hand-side range, `--goal` seeks, `--eval` evaluates a formula against the workbook, and `--convert` writes the workbook out in the format the extension names. The spec strings are the terminal's: `--solve 'max B4 vars A4:A5 st D4:D6'` is the argument of `:opt`, parsed by the same module, so a spec that works in one works in the other.

  Three decisions worth stating. **Nothing is written unless asked** -- solve and goal report and leave the file alone, `--apply` writes into the grid, and that only reaches disk with `--convert`; there is deliberately no in-place overwrite, because a batch tool that rewrites the workbook you questioned is one you cannot safely run twice. **JSON is the default output** and its schema is documented as a contract (`docs/reference/cli.md`), dumped with `allow_nan=False` so a non-finite number is a loud failure rather than `Infinity` tokens no conforming parser accepts; every numeric field goes through the same null-for-non-finite rule. **The exit code carries the answer**: 0 succeeded, 2 ran and came back negative (infeasible, unbounded, did not converge), 1 never ran. In CI that is the difference between "the plan is infeasible, alert someone" and "the job is broken".

  Three new core modules, all of which the architecture guard now proves import without pulling in curses -- if they did, `--solve` would need a terminal. `gridcalc.optspec` holds the `:opt`/`:goal` grammar, moved out of `tui/solve.py` (which imports curses) so both frontends parse one implementation rather than two that start identical. `gridcalc.report` holds the JSON shapes, which `web.Api` now calls instead of building its own by hand. `gridcalc.cli` is the mode itself. 37 new tests.

- **xlsx dates are read, rendered, written back, and understood by criteria.** A date in a spreadsheet is a number wearing a number format -- `2024-01-01` is `45292`, and only the format says it is a date at all -- so a reader that drops the format turns a date column into five-digit floats, which is what the limitations page used to promise. `_core.xlsx_read` now reports each cell's number-format code and numFmtId alongside its text; `gridcalc.dates` decides which codes mean "date" and renders serials through them; `Cell.fmtstr`, which already carried a Python numeric spec, carries the format code too, and the display layer picks the language at render time. Both frontends got it at once, because both format through `display.cell_text`.

  `:f yyyy-mm-dd` needed no new code path -- `apply_format` already routed an unrecognised spec to `fmtstr` -- so the same command sets a date format in the terminal and the desktop app. `xlsxsave` writes the format back, and `COUNTIF`/`SUMIF` convert a date operand (`">1/1/2020"`) to a serial before comparing, which is the difference between a right answer and a string comparison where `"9/1/2020"` sorts after `"10/1/2020"` and the count is quietly wrong.

  Classification is deliberately conservative: a format must contain a real date token, evaluated after quoted literals and colour sections are stripped, so `0" days"` stays a number. A misread in that direction turns a price into a year. The epoch is 1899-12-30, which matches Excel for every serial above 60 -- the range every real workbook lives in -- rather than reproducing its phantom 1900-02-29. The conversions moved out of `libs/xlsx.py` into `gridcalc.dates` so the function library, the display layer and the criteria parser share one copy of that offset. 71 new tests.

- **The docs cannot go on denying a feature that shipped.** `docs/reference/limitations.md` listed `LAMBDA`, its higher-order helpers and `OFFSET` as unsupported and said dynamic-array results pack into their origin cell; `docs/topological.md` opened with "proposal, not implemented". All of them had shipped, two releases earlier in some cases, and a reader who trusted either page concluded the feature was missing. `tests/test_docs_conformance.py` now asks the engine rather than the page: every function the limitations page names in a bullet is called with no arguments, and an unknown name answers `#NAME?` where a known one gets as far as its arguments and answers `#VALUE!`, which separates the two without a registry to consult -- there is no single registry, since `LET` and `LAMBDA` are special forms in `_eval_call` and only the library functions are in `BUILTINS`. One of the four guards the probe itself, so a probe that answered "absent" for everything cannot quietly make the others vacuous. Design notes are in scope here where the command tests exclude them: a status line is a claim about the code, not a proposal. 4 new tests.

- **The web bridge's file API rests on two properties, and they were prose.** `Api.save` and `open_file` deliberately take any path the caller passes -- see the "Trust model" section of `gridcalc/web`'s module docstring -- and the argument that this is a feature rather than a hole depends on the window being built from inlined HTML rather than a URL, and on cell content rendering as escaped React text nodes. Either one quietly changing would turn the bridge into an arbitrary local file read/write primitive for whatever the view then loaded, with nothing failing. Both are now tests. 2 new tests.

### Changed

- **uv builds against a managed interpreter, not whichever one is already installed.** `make build` on a fresh clone failed in CMake with `Could NOT find Python (missing: Interpreter Development.Module)`. The cause is not the version: uv only *downloads* an interpreter when nothing compatible is present, so a system `/usr/bin/python3.14` satisfied the request and was used, and a distro python splits its development headers into a separate package the extensions cannot build without. `pyproject.toml` now sets `python-preference = "only-managed"`, so uv fetches its own interpreter -- which always carries the headers -- and a clone builds with no prerequisite beyond uv itself. Verified from scratch with no managed 3.14 present: uv downloads 3.14.7 and `make build`, `test`, `typecheck` and `docs` all run with no argument. CI already did the equivalent by calling `uv python install` explicitly, which is why it never saw this. `.python-version` stays at 3.14 and `requires-python` still spans 3.10-3.14; a contributor who wants their own interpreter can pass `--python-preference system` once its `-dev` package is installed.
...
- **Copying a cell containing a tab or a newline to the OS clipboard rewrote it.** `rows_to_tsv` replaced both with spaces, on the reasoning that TSV has no quoting -- so a label holding either came back changed, silently, and only for the user who happened to have one. A copy-paste that corrupts a cell is worse than one that fails. Cells needing it are now wrapped in double quotes with their own quotes doubled (RFC 4180, which is also what Excel, Numbers and Sheets put on the clipboard for such a cell, so this is not a gridcalc dialect), and `tsv_to_rows` understands the quoting on the way back, including a field that spans newlines. A cell containing a bare `"` has to be quoted too: `"x"` written verbatim would come back as `x`, because nothing distinguishes an unquoted quote from a field the writer quoted. Cells needing none of it still cross verbatim, and the parser is pinned byte-for-byte against the old split-on-delimiters one for every input with no quote in it -- trailing empty rows and cells included -- so anything already sitting in a clipboard keeps meaning what it meant. Junk after a closing quote and an unterminated quoted field both keep the user's bytes rather than dropping them. 28 new tests.
