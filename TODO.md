...
- [ ] **Remaining `libs/xlsx.py` Excel-fidelity gaps.** The lookup / criteria / conditional-aggregate audit sweep is done (see CHANGELOG). Left open, each gated on a missing primitive or genuinely ambiguous in Excel: numeric-vs-text criteria coercion (`COUNTIF({1,2,"3"}, 3)`); and `SUMIF`/`COUNTIF` with a `sum_range` shorter than the criteria range (Excel resizes from the top-left, which needs reference rather than materialised-`Vec` semantics -- now unblocked, since `OFFSET` brought the `Reference` type into the value system).

- [x] The aggregates take exactly one argument -- `SUM`/`AVG`/`MIN`/`MAX`/`COUNT`/`AVERAGE` are variadic and flatten. See CHANGELOG.

- [ ] **`COUNT` and `COUNTA` count blank cells inside a range.** `=COUNT(A1:C3)` over a range holding one number, one label and seven empties answers 8 and `=COUNTA` answers 9; Excel answers 1 and 2. Blank cells materialise into the range `Vec` as `0.0`, so nothing downstream can tell an empty cell from a zero -- the fix is in range materialisation, not in the counting functions. Found while making the aggregates variadic.
...
- [ ] **Tag names and the publish trigger disagree, so no tag has ever published.** Every tag in the repo is bare (`0.5.1`, `0.5.0`, ... nine of them), `make release` creates `v$version`, and `build-publish.yml` fires on `tags: - "v*"`. No existing tag matches the trigger; every run of that workflow to date is a `workflow_dispatch`, so publishing has been manual and the automatic path has never once been exercised. Had the 0.6.0 release completed it would have been the first tag to match, meaning the first real test of that workflow would have been a live publish. Pick one convention and make the Makefile and the workflow agree; the `v` prefix is the cheaper side to keep, since only the historical tags disagree with it.

- [ ] **0.6.0 is bumped but not tagged.** `pyproject.toml` says `0.6.0` (commit 48554c9, whose message is `make release`'s exact wording and which touched only that file), but `v0.6.0` exists neither locally nor on origin, and `0.5.1` is still the newest tag. Either the release stopped after the commit or the tag was made and removed. Decide whether 0.6.0 ships as tagged or the version rolls forward, and settle the item above first -- tagging it under the current setup is what would trigger the untested publish path.
