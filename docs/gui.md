...
- **Step 3 (web editable grid): started.** `gridcalc/web/` is the spike, behind the optional `web` extra:
  ...
  **Undo / redo** (Ctrl+Z / Ctrl+Shift+Z / Ctrl+Y) reuse the engine-adjacent `UndoManager`, which was promoted out of `tui/undo.py` into a frontend-neutral `gridcalc/undo.py` (guarded curses-free, like `display.py` / `loader.py`; the TUI re-imports it). The `Api` holds one `UndoManager` and saves state before each mutation -- a cell snapshot before `set_cell`, the region before `clear_range`/`fill`, a whole-workbook snapshot (`save_grid`: every sheet, names and mode) before `paste` and sheet operations -- and undo recomputes derived cells through `recalc`, so a formula's dependents revert correctly.

  **Save** (Ctrl+S). `Api.save(path)` writes the workbook via `loader.save_workbook` (format by extension, shared with `:w` and `--convert`) to the given path or the loaded filename; the demo (no filename) returns `needs_path`, and the client falls back to `Api.save_dialog`, which opens pywebview's native save dialog (`create_file_dialog`, using the window handle `run()` stashes on the `Api`). A short "saved" flashes in the toolbar. Round-trip is real: a unit test saves through the `Api` and reloads the file, confirming edits and formulas survive.

  **Open a different workbook** (Ctrl+O). `Api.open_dialog` opens pywebview's native open dialog and `Api.open_file(path)` swaps the loaded model in via the shared `loader.load_workbook` (now `.json`/`.xlsx`/`.csv`). ~~formulas-only for JSON so opening a file never runs code~~ A JSON file with code now goes through the trust dialog. Opening any JSON file still `eval()`s its PYTHON-mode formulas (`docs/security-plan.md`, "Known gaps"). Because a different sheet is now in scope, `open_file` resets the per-workbook UI state that would otherwise dangle -- it clears the `UndoManager` and drops the copy buffer -- and best-effort retitles the window; the client asks before discarding unsaved changes, re-fetches `dims`, redraws the tabs, and moves the cursor back to A1. A failed load, including any exception the parser or recalc raises, returns `{ok: false, error}` and leaves the current workbook untouched. A successful one returns the loader's `warnings` when there are any; the client does not show them yet.
