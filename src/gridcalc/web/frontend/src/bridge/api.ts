...
export const bridge: PywebviewApi = {
  ...
  recalc: () => api().recalc(),
  list_names: () => api().list_names(),
  set_name: (name, rng) => api().set_name(name, rng),
  delete_name: (name) => api().delete_name(name),
  ...
  insert_rows: (at, count) => api().insert_rows(at, count),
  insert_cols: (at, count) => api().insert_cols(at, count),
  delete_rows: (r0, r1) => api().delete_rows(r0, r1),
  delete_cols: (c0, c1) => api().delete_cols(c0, c1),
  undo: () => api().undo(),
  redo: () => api().redo(),
